...
fun MatchesByIdItems(
    ...
    state.matches?.let { match ->
        Column(
            ...
            Divider(color = Color.LightGray)
            val convertedTime = Constants.toLocalDateTime(match.datetime, "h:mm a, MMM dd")
            val output = convertedTime.month.getDisplayName(TextStyle.SHORT, Locale.ENGLISH) + ", " + convertedTime.year
            VerticalDividerTwoRows(
                title1 = "Time",
                title2 = output,
                screenSize1 = 0.5f,
